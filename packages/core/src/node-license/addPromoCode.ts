import { ethers } from 'ethers';
import { NodeLicenseAbi } from '../abis/index.js';
import { config } from '../config.js';

/**
 * Adds a promo code if the signer has the DEFAULT_ADMIN_ROLE.
 * @param promoCode - The promo code to add.
 * @param recipient - The recipient address.
 * @param signer - The signer to interact with the contract.
 * @returns The transaction receipt.
 */
export async function addPromoCode(
    promoCode: string,
    recipient: string,
    signer: ethers.Signer,
): Promise<ethers.TransactionReceipt> {

    // Create an instance of the NodeLicense contract
    const nodeLicenseContract = new ethers.Contract(config.nodeLicenseAddress, NodeLicenseAbi, signer);

    // Add the promo code
    const addPromoCodeTx = await nodeLicenseContract.createPromoCode(promoCode, recipient);

    // Wait for the transaction to be mined and get the receipt
    const txReceipt = await addPromoCodeTx.wait();

    return txReceipt;
}
