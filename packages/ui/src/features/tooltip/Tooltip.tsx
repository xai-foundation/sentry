...
export const Tooltip = ({ header, body, body2, body3, banner, bannerTitle, bannerValue, width = 443, position = "start", side = "bottom", open, extraClasses, children }: TooltipProps) => {
	return (
		<TooltipPrimitive.Provider delayDuration={0}>
			<TooltipPrimitive.Root open={open}>
				...
				<TooltipPrimitive.Content
					...
					side={side}
					sideOffset={12}
					className={`relative -top-1 ${position === "start" ? "left-[-0.39rem] items-start" : "right-[-0.39rem] items-end"} flex-col flex text-black shadow-md z-50 ${extraClasses?.content}`}
				...
				>
					...
					<div className={`relative w-full py-3 px-4 bg-white border border-[#D4D4D4] shadow-lg`}>
						...
						{body && <p className={`text-[15px] font-light ${extraClasses?.body}`}>{body}</p>}
