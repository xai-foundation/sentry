...
export function TableRowRewards({
  ...
  return (
    <>
      ...
      {/**/}
      <td className="whitespace-nowrap text-graphiteGray lg:p-4 sm:p-2 text-left sm:text-sm lg:text-base">
        {showTableKeys ?
          <Link
...
export function TableHead({
  ...
  return (
    <th
      colSpan={index === 3 ? 4 : 1}
      scope="col"
      className={`text-left ${index === 1 && "lg:table-cell sm:hidden"
      } lg:py-4 sm:py-2 bg-crystalWhite font-medium lg:text-base sm:text-sm
        ${index === 2 ? "w-full" : ""}
        ${index === 0 ? "pl-2 lg:pl-4" : ""}
        `}
      key={index}
    >
...
export function TableRowStaked({ value, customClass, poolAddress }: { value: string, customClass?: string, poolAddress?: string }) {
  return (
    <td
      className={`lg:whitespace-nowrap lg:py-4 lg:text-base text-graphiteGray text-left sm:text-xs pr-2 lg:pr-20  ${customClass}`}>
      <Link href={`/pool/${poolAddress}/summary`} className="w-full" >
...
export function TableRowStakedV1({ value, customClass }: { value: string, customClass?: string }) {
  return (
    ...
    <td className="whitespace-nowrap text-graphiteGray lg:px-4 py-4 text-left sm:text-sm lg:text-base">
      <div className="w-full">
        <div className="flex sm:flex-col lg:flex-row items-center justify-between">
          {totalStaked > 0 && <span className="justify-self-start sm:hidden lg:table-cell">
            {pool ? <Link
              href={`/pool/${pool.address}/summary`}>{formatCurrencyNoDecimals.format(pool.userStakedEsXaiAmount!)} esXAI</Link> : value}
          </span>}
          {pool ?
