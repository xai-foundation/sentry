...
export function SubmittingAndClaiming(deployInfrastructure, poolConfigurations) {
	...
	return function () {
		...
		it("Operator should be able to submit multiple assertions & bulk claim for multiple winning keys in a pool (multiple license holders)", async function () {
			...
			// Make winning state root for both of addr2's keys
			const challengeId = 0;
			const winningStateRoot = await findWinningStateRoot(referee, [addr2MintedKeyId, addr3MintedKeyId], challengeId);
			...
			// Submit a winning hash
			await referee.connect(operator).submitMultipleAssertions([addr2MintedKeyId, addr3MintedKeyId], challengeId, winningStateRoot);

			// Grab both the submissions & expect them to both be eligible
			const submission1 = await referee.getSubmissionsForChallenges([challengeId], addr2MintedKeyId);
			const submission2 = await referee.getSubmissionsForChallenges([challengeId], addr3MintedKeyId);
			expect(submission1[0].eligibleForPayout).to.equal(true);
			expect(submission2[0].eligibleForPayout).to.equal(true);
			...
			// Bulk reward claim as operator
			await referee.connect(operator).claimMultipleRewards([addr2MintedKeyId, addr3MintedKeyId], challengeId, stakingPoolAddress);
