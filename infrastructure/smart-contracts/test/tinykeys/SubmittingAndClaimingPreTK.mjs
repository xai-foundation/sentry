...
export function SubmittingAndClaimingPreTK(deployInfrastructure, poolConfigurations) {
	const {
		validShareValues,
		poolMetaData,
		poolSocials,
	...
	return function () {
		it("Pool owner should be able to submit multiple assertions & claim for multiple winning keys in a pool (single license holder)", async function () {
			...
			const stakingPoolAddress = await createPool(poolFactory, addr1, [1], noDelegateOwner);
			...
			// Claim reward claim as operator
		...
		it("Pool delegate should be able to submit multiple assertions & claim for multiple winning keys in a pool (multiple license holders)", async function () {
