 ...
 */
export function esXaiTestsPreTK(deployInfrastructure) {
    return function() {
        it("Check calling the initializer is not allowed afterwards", async function() {
            const {esXai, esXaiMinter, deployer} = await loadFixture(deployInfrastructure);
            const expectedRevertMessage = "Initializable: contract is already initialized";
            await expect(esXai.connect(esXaiMinter).initialize(await deployer.getAddress(), BigInt(500))).to.be.revertedWith(expectedRevertMessage);
        })
