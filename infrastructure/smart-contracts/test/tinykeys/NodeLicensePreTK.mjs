...
export function NodeLicensePreTK(deployInfrastructure) {
    return function() {

        it("Check calling the initializer does not exist and is not callable", async function() {
            ...
            try {
                await expect(nodeLicense.connect(nodeLicenseDefaultAdmin).initialize()).to.be.revertedWith(expectedRevertMessage);
            } catch (error) {
                console.log("Confirmed Initialize Fails: ", error.message);
            }
        })
