...
async function main() {
    ...
    for (let i = 0; i < walletCount; i++) {
        ...
        console.log(`Loaded wallet at index ${i}: ${adminWalletAddress}, currentNonce: ${WALLET_TO_NONCE[adminWalletAddress]}, currentBalance: ${Number(await ethers.provider.getBalance(adminWalletAddress)) / 10 ** 18}`);
    }
...
const processTransaction = async (signer) => {
    ...
    console.log("FeedData: ", feeData.maxFeePerGas, maxFeePerGas < feeData.maxFeePerGas);
