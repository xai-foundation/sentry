...
contract NodeLicense11 is ERC721EnumerableUpgradeable, AccessControlUpgradeable  {
    ...
    */
     ...
     * @param from The address to transfer the tokens from, this requires approval first
     * @param to The address to transfer the tokens to
     * @param tokenIds The tokenIds to transfer
     * @param transferId The transfer id that is used to not allow multiple transfers with the same id
     */
    function adminTransferBatch(
        ...
        uint256 stakedCount = Referee10(refereeAddress).assignedKeysOfUserCount(from);
        if(stakedCount + tokenIdsLength > balanceBefore) revert CannotTransferStakedKey();

        Referee10(refereeAddress).updateBulkSubmissionOnTransfer(from, to);
        emit AdminTransferBatch(msg.sender, transferId);
    }
    ...
    function transferFrom( 
        ...
        uint256 stakedCount = Referee10(refereeAddress).assignedKeysOfUserCount(from);
        if(stakedCount >= balanceBefore) revert CannotTransferStakedKey();
    }
    ...
    /**
     * @notice Transfer keys staked in a pool from onw owner to another, require TRANSFER_ROLE on msg.sender
     * The Ownership of the individual keyIds will be updated the same as on a regular transfer.
     * The key amount will be unstaked
     * @param from The address to transfer the tokens from, this requires approval first
     * @param to The address to transfer the tokens to
     * @param poolAddress The address of the pool the keys are staked in by the from address
     * @param tokenIds The tokenIds to transfer
     */
