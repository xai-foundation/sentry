...
contract Referee6 is Initializable, AccessControlEnumerableUpgradeable {
     ...
     */
    function setChallengerPublicKey(
        bytes memory _challengerPublicKey
     ...
     */
    function setNodeLicenseAddress(
    ...
    function submitAssertionToChallenge(
        ...
        address licenseOwner = NodeLicense(nodeLicenseAddress).ownerOf(
            _nodeLicenseId
        );
    ...
    function _submitAssertion(
        ...
        (bool hashEligible, ) = createAssertionHashAndCheckPayout(
            _nodeLicenseId,
            _challengeId,
            _getBoostFactor(stakedAmount),
            _confirmData,
    ...
    function claimReward(uint256 _nodeLicenseId, uint256 _challengeId) public {
        ...
        if (
            block.timestamp >= challengeToClaimFor.createdTimestamp + 180 days
        ) {
        ...
        Submission memory submission = submissions[_challengeId][
            _nodeLicenseId
        ];
    ...
    function expireChallengeRewards(uint256 _challengeId) public {
        ...
        require(
            block.timestamp >=
                challenges[_challengeId].createdTimestamp + 180 days,
            "29"
        );
     ...
     */
    function updateMaxStakePerLicense(
        uint256 newAmount
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateStakingTier(
        ...
        if (index == 0) {
            ...
            require(
                stakeAmountTierThresholds[lastIndex - 1] < newThreshold,
                "35"
            );
        } else {
            require(
                stakeAmountTierThresholds[index + 1] > newThreshold &&
                    stakeAmountTierThresholds[index - 1] < newThreshold,
                "36"
            );
        }
